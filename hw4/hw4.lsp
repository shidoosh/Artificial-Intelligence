...
(defun at-most-one-color-helper (n c next k)
  (cond ((> next k) nil)
        (t (append (list (list (- (node2var n c k)) (- (node2var n next k)))) (at-most-one-color-helper n c (+ next 1) k)))
  )
)

(defun at-most-one-color (n c k)
	(cond ((= c k) nil)
        	(t (append (at-most-one-color-helper n c 1 k) (at-most-one-color n (+ c 1) k)))
  	) 
)
